...
group_by(block) %>%
  mutate(krate_kgha = sample(krate_kgha)) %>% #sample = shuffle
...
How many rows do we have in the object above? Why? 12 - we only have K now (4 blocks x 3 K levels)
...
group_by(wplot) %>%
  mutate(nrate_kgha = sample(nrate_kgha)) %>% #nrate level seen only once in a whole plot
  # Creating SP plot numbers 
  mutate(splot = paste0(wplot, "-",1:3)) %>% #paste0 - fn to combine strings
ungroup()
...
spp_rcbd_rand <- wp_rcbd_rand %>%
  right_join(sp_rcbd_rand) %>% #right join coz we want to have 36 rows, which is in sp_rcbd_rand
