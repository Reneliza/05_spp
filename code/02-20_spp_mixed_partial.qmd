...
```{r glimpse}
glimpse(spp_rcbd_df) #anova - categorical; regression - numerical
```
# c) Wrangling
```{r rcbd_dfw}
...
To account for more than one variance component (i.e., random effects and error), we can use function `lmer()` from package `lme4`.  lme4 - popular mixed model package
...
spp_rcbd_mix_mod <- lmer(data = spp_rcbd_dfw,
                         formula = yield_kgha ~ 
                           ...
                           ) #(1|block / krate_kgha) = telling the model that krate is in whole or main plot, thus, it is split plot - the smallest level of nesting, you do not specify because theor errors are in the residual
...
randeff_blockkrate #12 rows coz you have 4 blocks and 3 k rate levels
```
...
What happens with the emmean error if we ask for the emmean of the main effects of K and N rate separately? error in whole plot (krate) is lower than the interaction

```{r k rate means}
emmeans (spp_rcbd_mix_mod, ~krate_kgha) #error coming from whole plot
```
The SE of the whole-plot treatment factor (k rate) is **232**.

What about for the split-plot treatment factor (n rate)?  

SE is 209, n rate means SE are lower than that of SE emmans of krate. emmeans of krate is higher, thus, you have more power in the split-plot level coz you have more number of replications
...
Letters changed from RCBD coz we're using different standard errors, thus, running and specifying the proper model will have an impact on your results.

Thus, plan your design well before you commit to your study. 
